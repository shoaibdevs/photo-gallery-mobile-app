...
import { setPopupAlbum, setPopupDeleteAlbum } from '../../redux'
...
export default function popupAlbums(props) {
    const dispatch = useDispatch()
    const { user, popupDeleteAlbum } = useSelector(state => state.user)
    ...
    return (
        <>
            {Object.keys(item).length > 0 ?
                <View>
                    <Modal animationOutTiming={500} animationInTiming={500} onBackdropPress={() => dispatch(setPopupAlbum(false))} backdropColor={'rgb(220,219,223)'} isVisible={popupAlbums}>
                            ...
                            <View style={styles.container}>
                                <View style={{ width: '85%', alignSelf: 'center', top: 20 }}>
                                    ...
                                    <TouchableOpacity onPress={() => {
                                        navigation.navigate('PreviewAlbum', { currentAlbum });
                                        dispatch(setPopupAlbum(false));
                                    }} >
                                        <View style={{ flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-end' }}>
                                            <Text style={{ fontFamily: 'Arimo-Bold', fontSize: 17, color: colors.register, textAlign: 'right', right: 10 }}>{'תצוגה מקדימה והזמנת אלבום'}</Text>
                                            <Image style={{ width: 25, height: 25 }} source={require('../../../assets/images/iconsCirclePurple.png')} />
                                        </View>
                                    </TouchableOpacity>
                                    {item.max_images != '10' && item.status === '1' ? (
                                        <>
                                            <VerticalSpace height={0.03} />
                                            ...
                                            <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-end' }}
                                                onPress={() => {
                                                    navigation.navigate('EnlargeAlbumCapacity', { currentAlbum: item, photoFromGallery: false, returnToAlbumPreview: true })
                                                    dispatch(setPopupAlbum(false));
                                                }}
                                            >
                                                <Text style={{ fontFamily: 'Arimo-Regular', fontSize: 17, textAlign: 'right', right: 10 }}>{'הגדלת מספר התמונות באלבום'}</Text>
                                                <Image style={{ width: 25, height: 25 }} source={require('../../../assets/images/iconPictures±ForAlbumMenu.png')} />
                                    ...
                                    <View style={[styles.line, { borderWidth: 0.6, borderColor: colors.textInputBorder }]} />
                                    <VerticalSpace height={0.03} />
                                    <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-end' }} onPress={() => dispatch(setPopupAlbum(false))}>
